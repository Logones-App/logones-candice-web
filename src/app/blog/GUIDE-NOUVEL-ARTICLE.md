...
`src/app/blog/content/<theme>/<slug>.tsx`
...
Dans `src/app/blog/lib/articles.ts` :
...
C'est tout pour que l'article **existe** : la route `/blog/<theme>/<slug>`,
...
- `publishedAt` **dans le futur** → en **production**, article masqué partout + URL en **404** jusqu'à la date.
...
- `draft: true` → jamais visible en production, quelle que soit la date.
- Pour une heure quasi exacte, on peut baisser `revalidate` (ex. 60).

### Aperçu avant publication

- **En local (`npm run dev`)** : `PREVIEW_MODE` est actif → **TOUS** les articles sont
  visibles (programmés + brouillons), avec un bandeau orange « Aperçu — non publié » sur
  l'article et un badge « Programmé / Brouillon » dans la liste du thème.
- **En production** : `PREVIEW_MODE` est faux → seuls les articles réellement en ligne
  s'affichent. Logique : `isLive()` = statut réel (prod) ; `isVisible()` (interne) = `PREVIEW_MODE || isLive()`.
- Donc : préparer/relire les articles en local, fixer leur `publishedAt`, et ils sortiront
  seuls à leur date en prod.
