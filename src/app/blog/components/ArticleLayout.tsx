...
import { getRelatedArticles, isLive } from "../lib/articles";

export const SITE_URL = "https://logones.fr";

export function ArticleLayout({ article }: { article: Article }) {
  const related = getRelatedArticles(article);
  const url = `${SITE_URL}/blog/${article.theme}/${article.slug}`;
  // En prod cet article serait-il en ligne ? Sinon on l'affiche en aperçu (dev).
  const previewOnly = !isLive(article);
      ...
      },
      {
        ...
        itemListElement: [
          ...
          },
          {
            ...
            item: `${SITE_URL}/blog/${article.theme}`,
          },
          {
            "@type": "ListItem",
            position: 3,
  ...
  return (
    <article style={{ backgroundColor: C.bg }}>
      ...
      {previewOnly && (
        ...
        >
          Aperçu — article non publié
          {article.draft ? " (brouillon)" : ` · programmé le ${article.dateLabel}`}.
          Invisible en production jusqu&apos;à sa date.
        </div>
      )}
      ...
      >
        <div className="mx-auto" style={{ maxWidth: "820px" }}>
          ...
          >
            <Link href="/blog" style={{ color: C.textOnDarkSubtle }}>
              Blog
            </Link>
            <ChevronRight size={13} aria-hidden />
            <Link
              href={`/blog/${article.theme}`}
